   ...
   Ha ocurrido una excepción: " . $ex->getMessage() . "</div>";
    ...
    <div class="container-fluid">
        ...
        function findAllAuthors(): array
        {
            ...
            try {
                $conProyecto = getConnection();
                $consulta = "SELECT author_id, CONCAT(COALESCE(first_name, ' '), COALESCE(middle_name, ' '), COALESCE(last_name, ' ')) AS completeName 
                FROM authors 
                ORDER BY  last_name";
                ...
                echo "<div class='alert alert-danger' role='alert'> Excepcion" . $e->getMessage() . "</div>";
            } finally {
                if ($resultado != null)
                    $resultado->close();
            }
